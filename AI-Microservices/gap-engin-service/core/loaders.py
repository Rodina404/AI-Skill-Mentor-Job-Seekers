...
from core.role_library import normalize_skill_name
...
# Cached state
_roles: list[dict] = []
_title_index: dict[str, dict] = {}  # normalized_title â†’ role
...
def load_job_title_index() -> tuple[list[dict], dict]:
    ...
    Returns:
        (roles, title_index) where title_index maps normalized title â†’ role dict.
    """
...
def find_best_match(title: str, semantic_threshold: float = 0.74) -> Optional[tuple[dict, float, str]]:
    ...
    Strategy:
      1. Exact match on normalized title â†’ confidence 1.0
      2. Semantic similarity â‰¥ threshold AND token overlap > 0 â†’ confidence = similarity
    ...
    if best_sim >= semantic_threshold and overlap > 0:
        logger.debug(f"Semantic match for '{title}' â†’ '{best_role['title']}' (sim={best_sim:.3f})")
