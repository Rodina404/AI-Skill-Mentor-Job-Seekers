...
async def startup_event():
    """Initialize the job recommender on startup"""
    if not recommender.initialize():
        logger.error("Failed to initialize job recommender")
        raise RuntimeError("Job recommender initialization failed")
...
@router.post('/debug-init-job')
async def debug_initialize(reinitialize: bool = False):
    """Debug endpoint to force recommender initialization and report model files"""
    try:
        # Optionally re-run initialization
        if reinitialize:
            recommender.is_initialized = False

        ok = recommender.initialize()

        # Check model files in expected model path
